...
class PublishVoiceViewController: BaseViewController {
    ...
    override func viewDidLoad() {
        ...
        textview.snp.makeConstraints { (make) in
            make.top.equalTo(10)
            make.size.equalTo(CGSize(width: self.view.frame.width-20, height: 356))
        ...
        PublishVoice.VoiceButton.addTarget(self, action: #selector(VoiceButtonClick), for: .touchDown)
 ...
 */
    ...
    @objc func VoiceButtonClick() {
        if audioEngine!.isRunning {
            ...
            PublishVoice.VoiceButton.isEnabled = false
            PublishVoice.VoiceButton.setTitle("1", for: .normal)
            PublishVoice.VoiceButton.isEnabled = true
        } else {
            startRecording()
            PublishVoice.VoiceButton.setTitle("0", for: .normal)
        }
    }
...
extension PublishVoiceViewController: SFSpeechRecognizerDelegate {
    /// 可用性状态改变时被调用
